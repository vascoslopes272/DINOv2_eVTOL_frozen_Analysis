...
deterministic rules are compared on the same aircraft:

    exp1_view_first   best view, then best flight state
    exp2_state_first  best flight state, then best view
    exp3_main         the figure the labeller marked as main
    exp4_slots        one figure per view slot (Perspective, Plan, Side), concatenated
...
Output: ``<paths.pipeline_root>/view_state_experiments/``::
    ...
    aircraft_common.csv       aircraft in exp1, exp2, exp3 and exp4 (Perspective + Plan)
...
# Labelled acState id -> state4 for a VARIANT aircraft. 'HoverCruise' is the id
# of the wizard option "Invariant"; Ground/Unclear/NonApplicable are retired ids
# that old records may still hold. A blank label is "Missing" (never imputed).
STATE_VARIANT = {
    "Hover": "Hover", "Cruise": "Cruise",
    "Transition": "Other", "Other": "Other", "HoverCruise": "Other",
...
def select_main(w: pd.DataFrame, t_all: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    """exp3: the human-marked main figure; aircraft with 0 or >1 mains go to conflicts.
    ``t_all`` (scope column included) tells whether a missing main was a part figure."""
    mains = w[w["is_main"]]
    ...
    return ok[MANIFEST_COLS].sort_values("aircraft_id").reset_index(drop=True), conflicts
...
def select_slots(w: pd.DataFrame, canonical: str = CANONICAL_STATE) -> pd.DataFrame:
    ...
    r = _ranked(w[w["view4"].isin(SLOTS)], ["_view", "_state"], canonical)
    ...
    top["_slot"] = top["slot"].map({s: i for i, s in enumerate(SLOTS)})
...
def common_aircraft(exp1, exp2, exp3, exp4) -> List[str]:
    ids = set(exp1["aircraft_id"]) & set(exp2["aircraft_id"]) & set(exp3["aircraft_id"])
    return sorted(ids & slot_complete(exp4, SLOTS[:2]))
...
def build_matrix(manifest: pd.DataFrame, ids: List[str], slots: List[str] | None,
                 X: np.ndarray, meta: pd.DataFrame) -> Tuple[np.ndarray, List[str], pd.DataFrame, List[str]]:
    """Rows in ``ids`` order. ``slots=None``: one figure per aircraft; else the slot
    vectors concatenated in that order, dropping aircraft that miss a slot."""
...
def experiment_meta(name: str, manifest_file: str, slots: List[str] | None, emb: Dict[str, Any],
                    canonical: str, Xo: np.ndarray, dropped: List[str]) -> Dict[str, Any]:
    m = emb["manifest"]
    return {
        "experiment": name,
        ...
        "layer": emb["layer"],
        "pooling": emb["pooling"],
        "vectors": "L2-normalised per figure" + ("; slot vectors concatenated, not renormalised" if slots else ""),
        "slots": slots,
        "extraction_created": m["created"],
        "extraction_git_commit": m["git_commit"],
        "canonical_state": canonical,
        "restricted_to": "aircraft_common.csv",
...
def write_report(path: Path, *, ov: pd.DataFrame, ov_all: pd.DataFrame, n_common: int,
    ...
    L = [
        ...
        "",
        f"Share of aircraft for which two rules pick the same figure: on the **{n_common} common aircraft** "
        "(the comparison set), and, for context, on every aircraft the three single-figure rules share.",
        "",
        _md_table(ov.assign(set="common")),
        "",
        _md_table(ov_all.assign(set="all shared")),
        "",
        "Invariant aircraft carry one state on every figure, so exp1 and exp2 can only differ on variant aircraft.",
        "",
        f"**Caution:** the common set is small ({n_common} aircraft) because only {s['slot2_pct']} % of aircraft have "
        f"both a Perspective and a Plan figure; {len(small)} of {len(classes)} G1 classes have fewer than 20 aircraft in it "
        f"({', '.join(small)}).",
        "",
        "## Sources",
        "",
        ...
        "anything else (Transition, Other, the wizard's `HoverCruise` = Invariant option) → Other. "
        ...
        "exp3: the labeller's main figure. exp4: per slot, state then fig_number. "
    ...
    L += ["exp3 conflicts (left out of exp3):", "", _md_table(conflicts) if len(conflicts) else "none", ""]
    m = emb["manifest"]
    L += [
        ...
        "Every selected figure has a row; nothing was imputed. exp4 vectors are the slot vectors concatenated "
        "[" + ", ".join(SLOTS) + "] without renormalising.",
        "",
        _md_table(pd.DataFrame([{"folder": k, **v} for k, v in folders.items()])),
        "",
        "All folders are restricted to `aircraft_common.csv` and share its row order; `exp4_slots_3` additionally drops "
        "the aircraft with no Side figure (`dropped_ids.csv`).",
        "",
        "### G1 classes (common set)",
        "",
        _md_table(classes),
        "",
