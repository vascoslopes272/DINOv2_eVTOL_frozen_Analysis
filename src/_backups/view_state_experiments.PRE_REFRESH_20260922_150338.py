...
# SRW is variant (user, 2026-09-18: the rotor stops and becomes the wing in cruise),
# the same set as the wizard's isConvertibleArch(), which asks every SRW figure its state.
INVARIANT = {"RC", "MR", "SLC", "HB", "PFV", "TB", "PTC"}
VARIANT = {"TW", "TR", "DS", "CVT", "SRW"}
...
# Decided by decide_canonical() (Step 3.2; run of 2026-09-18, SRW variant): of 345
# variant aircraft, 215 have >=1 Perspective+Cruise figure and 209 >=1 Perspective+Hover.
...
def write_report(path: Path, *, ov: pd.DataFrame, n_common: int, n_slots: Dict[int, int],
    ...
    L = [
        ...
        "variant codes (" + ", ".join(sorted(VARIANT)) + ") map Hover → Hover, Cruise → Cruise, "
        "Invariant (the drawing fits both states) → Both, blank → Missing, anything else (Transition, Other) → Other. "
        "The brief's TP is the wizard's TR; PTC is invariant (ruling 2026-09-17); SRW is variant, as in the wizard "
        "(ruling 2026-09-18).",
        "",
