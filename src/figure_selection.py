...
def load_master(cfg: Dict[str, Any]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    root = Path(cfg["paths"]["labelled_root"]) / "joined"
    master = pd.read_excel(root / "master_labels.xlsx", keep_default_na=False, na_values=[""])
    figures = pd.read_excel(root / "master_figures.xlsx", keep_default_na=False, na_values=[""])
